...
func (us *UserService) ChangePassword(id int, req ChangePassword) error {
	user, err := us.repo.GetByID(id)
	if err != nil {
	...
	err = us.repo.UpdatePassword(id, string(newPasswordHash))
	if err != nil {
		return fmt.Errorf("failed to update password: %w", err)
	}

	us.logger.WithField("user_id", id).Info("Password changed successfully")
